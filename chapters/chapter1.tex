...
\begin{example}\label{ex:function_space}
  The set of all $F$-valued functions defined on a non-empty set $S$, i.e., $\{ f : S \to F \}$, denoted by $\Map(S, F)$ or $F^S$, forms a linear space over $F$ with the following operations:
  \begin{align*}
    + : \Map(S, F) \times \Map(S, F) & \to \Map(S, F)                                                                    \\
    (f, g)                           & \mapsto (f + g) : S \to F, \quad (f + g)(s) = f(s) + g(s)                         \\
    \cdot : F \times \Map(S, F)      & \to \Map(S, F)                                                                    \\
    (\alpha, f)                      & \mapsto (\alpha \cdot f) : S \to F, \quad (\alpha \cdot f)(s) = \alpha \cdot f(s)
  \end{align*}
\end{example}
...
\begin{example}
  The set of all finitely supported $F$-valued functions defined on a non-empty set $S$, i.e., $\{ f : S \to F \mid f(s) \neq 0_F \text{ for only finitely many } s \in S \}$, denoted by $F[S]$ or $\Map_{\fin}(S, F)$ or $F^{(S)}$, forms a linear space over $F$ with the same operations as in the previous example.
\end{example}
...
\begin{example}
  Let $S$ be a non-empty set. For each $s \in S$, define the Kronecker delta function $\delta_s : S \to F$ by
  \begin{equation}\label{eq:delta_function}
    ...
    \end{cases}
  \end{equation}
  Clearly, $\delta_s$ is in $F[S]$ since it is finitely supported. The set $\delta_S = \{ \delta_s \mid s \in S \}$ is a linearly independent set in the linear space $F[S]$ over $F$. To show this, assume there exists a finite linear combination of other delta functions such that $\delta_s = \sum \alpha^t \delta_t$. Then we have $\delta_s(s) = 1$ and $\delta_s(s) = \sum \alpha^t \delta_t(s) = 0$, which shows a contradition. Moreover, it is a minimal spanning set of $F[S]$ since any finitely-supported function can be expressed as a finite linear combination of the functions in this set.
\end{example}
...
\begin{problem}\label{prob:function_space_homomorphisms}
  ...
  \begin{enumerate}
    ...
    \item Can we get a natural map from $\Map(X, F)$ to $\Map(Y, F)$ or from $F[Y]$ to $F[X]$ for any set map $T: X \to Y$ between two infinite sets $X$ and $Y$?
  \end{enumerate}
\end{problem}
