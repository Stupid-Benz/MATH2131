...
\begin{definition}[Isomorphisms]\label{def:isomorphism}
  An \emph{isomorphism} is a homomorphism $\phi : A \to B$ between two algebraic structures that has an inverse homomorphism $\phi^{-1} : B \to A$ such that $\phi \circ \phi^{-1} = \id_{B}$ and $\phi^{-1} \circ \phi = \id_{A}$.
\end{definition}
...
\begin{example}\label{ex:function_space}
  The set of all $F$-valued functions defined on a non-empty set $X$, i.e., $\{ f : X \to F \}$, denoted by $\Map(X, F)$ or $F^X$, forms a linear space over $F$ with the following operations:
  \begin{align*}
