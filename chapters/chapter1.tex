...
\begin{definition}[Monoid Homomorphism]\label{def:monoid_homomorphism}
  A \emph{monoid homomorphism} is a set map $\phi : M_1 \to M_2$ between two monoids $(M_1, \cdot)$ and $(M_2, \odot)$ which respects the \hyperref[def:monoidic_structure]{monoidic structure}, i.e., for all $a, b \in M_1$, we have
  \begin{itemize}
...
\begin{definition}[Group Homomorphism]\label{def:group_homomorphism}
  A \emph{group homomorphism} is a set map $\phi : G_1 \to G_2$ between two groups $(G_1, \cdot)$ and $(G_2, \odot)$ which respects the \hyperref[def:groupic_structure]{groupic structure}, i.e., for all $a, b \in G_1$, we have
  \begin{itemize}
    \item $\phi(a \cdot b) = \phi(a) * \phi(b)$;
...
\end{proposition}
\begin{proof}
  Let $\phi : G_1 \to G_2$ be a group homomorphism satisfying the first property. For any $a \in G_1$, we have
...
\begin{definition}[Ring Homomorphism]\label{def:ring_homomorphism}
  A \emph{ring homomorphism} is a set map $\phi : R_1 \to R_2$ between two rings $(R_1, +, \cdot)$ and $(R_2, \oplus, \odot)$ which respects the \hyperref[def:ringic_structure]{ringic structure}, i.e., for all $a, b \in R_1$, we have
  \begin{itemize}
...
\begin{definition}[Endomorphism]\label{def:endomorphism}
  An \emph{endomorphism} is a homomorphism $\phi : A \to A$ from an algebraic structure to itself.
\end{definition}
...
\begin{definition}[Endomorphism Ring]\label{def:endomorphism_ring}
  ...
  \begin{align*}
    + : \End{G} \times \End{G}     & \to \End{G}                                                                     \\
    (\phi, \psi)                   & \mapsto (\phi + \psi) : G \to G, \quad (\phi + \psi)(a) = \phi(a) + \psi(a)     \\
    \circ : \End{G} \times \End{G} & \to \End{G}                                                                     \\
    (\phi, \psi)                   & \mapsto (\phi \circ \psi) : G \to G, \quad (\phi \circ \psi)(a) = \phi(\psi(a))
  \end{align*}
  The identity element for the addition operation is the zero map $0 : G \to G$ defined by $0(a) = 0_G$ for all $a \in G$, where $0_G$ is the identity element of the group $(G, +)$. The identity element for the composition operation is the identity map $\id_G : G \to G$ defined by $\id_G(a) = a$ for all $a \in G$.
\end{definition}
...
\begin{definition}[Linear Structure]\label{def:linear_structure}
  ...
  \begin{align*}
    \cdot : F & \to \End(V)                                                              \\
    \alpha    & \mapsto \alpha \cdot : V \to V, \quad (\alpha \cdot)(v) = \alpha \cdot v
  \end{align*}
\end{definition}
...
\begin{example}\label{ex:function_space}
  The set of all $F$-valued functions defined on a non-empty set $X$, i.e., $\{ f : X \to F \}$, denoted by $\Map(X, F)$, forms a linear space over $F$ with the following operations:
  \begin{align*}
    + : \Map(X, F) \times \Map(X, F) & \to \Map(X, F)                                                                    \\
    (f, g)                           & \mapsto (f + g) : X \to F, \quad (f + g)(x) = f(x) + g(x)                         \\
    \cdot : F \times \Map(X, F)      & \to \Map(X, F)                                                                    \\
...
\begin{example}
  The set of all finitely supported $F$-valued functions defined on a non-empty set $X$, i.e., $\{ f : X \to F \mid f(x) \neq 0_F \text{ for only finitely many } x \in X \}$, denoted by $\Map_{\fin}(X, F)$, forms a linear space over $F$ with the same operations as in the previous example.
\end{example}
...
\begin{example}
  ...
  \begin{align*}
    + : F^n \times F^n                                                                                                  & \to F^n                                                                & \cdot : F \times F^n                                         & \to F^n                                                                              \\
...
\begin{example}
  Let $X$ be a non-empty set. For each $x \in X$, define the Kronecker delta function $\delta_x : X \to F$ by
  \[
    ...
    \end{cases}
  \]
  Clearly, $\delta_x$ is in $\Map_{\fin}(X, F)$ since it is finitely supported. The set $\{ \delta_x \mid x \in X \}$ is a linearly independent set in the linear space $\Map_{\fin}(X, F)$ over $F$. To show this, assume there exists a finite linear combination of other delta functions such that $\delta_x = \sum \alpha^y \delta_y$. Then we have $\delta_x(x) = 1$ and $\delta_x(x) = \sum \alpha^y \delta_y(x) = 0$, which shows a contradition. Moreover, it is a minimal spanning set of $\Map_{\fin}(X, F)$ since any finitely-supported function can be expressed as a finite linear combination of the functions in this set.
\end{example}
...
\begin{definition}[Sum of Linear Subspaces]\label{def:sum}
  ...
  \[
    W_1 + W_2 + \cdots + W_n = \left\{ w_1 + w_2 + \cdots + w_n \mid w_i \in W_i, i = 1, 2, \ldots, n \right\}.
  \]
\end{definition}

\begin{definition}[Direct Sum of Linear Subspaces]\label{def:direct_sum}
...
The equivalent definition of direct sum is that the intersection of any subspace with the sum of the other subspaces is trivial, i.e., for each $i = 1, 2, \ldots, n$,
\[
  W_i \cap \left( \sum_{j \neq i} W_j \right) = \{ 0_V \}.
\]
...
\begin{problem}\label{prob:function_space_homomorphisms}
  Let $F$ be a field and $X$ be a non-empty set. Recall that $\Map(X, F)$ is the set of $F$-valued functions on $X$ and $\Map_{\fin}(X, F)$ is the set of finitely-supported $F$-valued functions on $X$. Both $\Map(X, F)$ and $\Map_{\fin}(X, F)$ are linear spaces over the field $F$.

  Let $T: X \to Y$ be a set map and $T_*: \Map_{\fin}(X, F) \to \Map_{\fin}(Y, F)$ be the map such that 
  \[
  ...
  \begin{enumerate}
    \item Show that ${(1_X)}_* = 1_{\Map_{\fin}(X, F)}$ for all non-empty set $X$.
    \item Show that ${(TS)}_* = T_* S_*$ for all set maps $T$ and $S$ such that the composition $TS$ is defined.
    \item For any set map $T: X \to Y$, we have an induced map $T^*: \Map(Y, F) \to \Map(X, F)$ via the formula $T^*f = fT$. Show that $1_X^* = 1_{\Map(X, F)}$ and ${(TS)}^* = S^*T^*$.
    \item Can we get a natural map from $\Map(X, F)$ to $\Map(Y, F)$ or from $\Map_{\fin}(Y, F)$ to $\Map_{\fin}(X, F)$ for any set map $T: X \to Y$ between two infinite sets $X$ and $Y$?
  \end{enumerate}
\end{problem}
