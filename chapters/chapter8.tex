...
Let $V$ be a vector space over $F$ where the characteristic of $F$ is not 2. We define the double $D(V) = V \oplus V^*$. Then we have a natural symplectic form on $D(V)$ defined as:
\[
...
\]
Then we have the left group action of $\GL_m(F) \times \GL_n(F)$ on the set of $m \times n$ matrices $\Mat_{m \times n}(F)$ defined as:
\[
  (Q, P) \cdot A = Q A P^{-1}
\]
...
\]
Then we have the left group action of $\GL_n(F)$ on the set of $n \times n$ matrices $\Mat_{n \times n}(F)$ defined as:
\[
  P \cdot A = P A P^{-1}
\]
