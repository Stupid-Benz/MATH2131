...
As $\omega (x, y) = g(x, J(y))$ for all $x, y \in V_{\R}$, i.e., $\omega_{\musNatural} = g_{\musNatural} \circ J$, so $\omega$ is non-degenerate if $g$ is non-degenerate. Then we have the following commutative diagram:
\begin{center}
