...
\begin{proposition}[Universal Property of Free Vector Spaces]\label{prop:universal_property_free_vector_space}
...
\begin{definition}[Morphism]\label{def:morphism}
  Let $\Ca$ be a \hyperref[def:small_category]{small category}. A \emph{morphism} $f$ in $\Ca$ is a structure-preserving map from an object $X$ to an object $Y$, denoted by $f : X \to Y$, where $X, Y \in \Ca_0$. The object $X$ is called the \emph{source} of $f$ and the object $Y$ is called the \emph{target} of $f$.
\end{definition}
...
\begin{definition}[Product]\label{def:product}
  Let $X$ and $Y$ be two objects in a \hyperref[def:category]{category} $\Ca$. A \emph{product} of $X$ and $Y$ is an object $X \prod Y$ in $\Ca$ together with two \hyperref[def:morphism]{morphisms} $\pi_X : X \prod Y \to X$ and $\pi_Y : X \prod Y \to Y$ such that for any object $Z$ in $\Ca$ with two morphisms $f_X : Z \to X$ and $f_Y : Z \to Y$, there exists a unique morphism $f : Z \to X \prod Y$ making the following diagram commute:
  \begin{center}
...
\begin{definition}[Coproduct]\label{def:coproduct}
  Let $X$ and $Y$ be two objects in a \hyperref[def:category]{category} $\Ca$. A \emph{coproduct} of $X$ and $Y$ is an object $X \coprod Y$ in $\Ca$ together with two \hyperref[def:morphism]{morphisms} $\iota_X : X \to X \coprod Y$ and $\iota_Y : Y \to X \coprod Y$ such that for any object $Z$ in $\Ca$ with two morphisms $f_X : X \to Z$ and $f_Y : Y \to Z$, there exists a unique morphism $f : X \coprod Y \to Z$ such that the following diagram commutes:
  \begin{center}
    \begin{tikzcd}[background color=ustblue!5]
...
\begin{definition}[Functor]\label{def:functor}
  Let $\Ca$ and $\D$ be two \hyperref[def:category]{categories}. A \emph{functor} $F : \Ca \to \D$ consists of the following data:
  \begin{itemize}
    \item A map $F_0 : \Ca_0 \to \D_0$ that assigns to each object $X$ in $\Ca$ an object $F(X)$ in $\D$;
    \item A map $F_1 : \Ca(X, Y) \to \D(F(X), F(Y))$ that assigns to each \hyperref[def:morphism]{morphism} $f : X \to Y$ in $\Ca$ a morphism $F(f) : F(X) \to F(Y)$ in $\D$;
  \end{itemize}
  satisfying the following properties:
  \begin{itemize}
    \item For any objects $X, Y, Z$ in $\Ca$ and morphisms $f : Y \to Z$, $g : X \to Y$, we have
          \[
...
\begin{example}
  ...
  \begin{center}
    ...
    \end{tikzcd}
  \end{center}
  The \emph{free vector space functor} $F[-] : \Set \to \Vect_F$ assigns to each set $X$ the free vector space $F[X]$ over $F$ generated by $X$, and to each set map $f : X \to Y$ the linear map $F[f] : F[X] \to F[Y]$ induced by $f$. The \emph{underlying set functor}, or \emph{forgetful functor}, $|-| : \Vect_F \to \Set$ assigns to each linear space $V$ over $F$ its underlying set $|V|$, and to each linear map $g : V \to W$ the set map $|g| : |V| \to |W|$ induced by $g$.
\end{example}
...
\begin{remark}
  ...
  If we consider $\phi : X \to |V|$ as an element in $\Set(X, |V|)$, then the corresponding element in $\Vect_F(F[X], V)$ is the unique linear map $\bar{\phi} : F[X] \to V$ induced by $\phi$.

  Note that $\iota \equiv \id_{F[X]}$ is the identity element in $\Vect_F(F[X], F[X])$, so it corresponds to an element in $\Set(X, |F[X]|)$, which is exactly the inclusion map $\iota : X \to |F[X]|$.
\end{remark}

\begin{definition}[Adjoint Functor]\label{def:adjoint_functor}
  Let $\Ca$ and $\D$ be two \hyperref[def:category]{categories}. A \hyperref[def:functor]{functor} $F : \Ca \to \D$ is called a \emph{left adjoint} of a functor $G : \D \to \Ca$, and $G$ is called a \emph{right adjoint} of $F$, denoted by $F \dashv G$, if for any object $X$ in $\Ca$ and any object $Y$ in $\D$, there exists a natural isomorphism:
  \[
...
\begin{example}
  The free vector space functor $F[-] : \Set \to \Vect_F$ is a left adjoint of the underlying set functor $|-| : \Vect_F \to \Set$, i.e., $F[-] \dashv |-|$. This follows from the natural isomorphism:
  \[
...
\begin{definition}[Endofunctor]\label{def:endofunctor}
  An \emph{endofunctor} is a \hyperref[def:functor]{functor} from a \hyperref[def:category]{category} to itself, i.e., $F : \Ca \to \Ca$.
\end{definition}
...
\begin{example}
  ...
  \]
  for all sets $Y$ and $Z$. This means that a set map $F : Y \times X \to Z$ corresponds to a set map $F_{\musNatural} : Y \to \Set(X, Z)$ such that a $y \in Y$ is mapped to a set map $F_{\musNatural}(y) : X \to Z$ defined by $F_{\musNatural}(y)(x) = F(y, x)$ for all $x \in X$.s
\end{example}
...
\[
  F[X \times Y] \equiv F[X] \otimes F[Y], \quad F[X \sqcup Y] \equiv F[X] \oplus F[Y].
\]
The left shows that the free functor $F[-]$ sends the product in $\Set$ to the tensor product in $\Vect_F$, and the right shows that it sends the coproduct in $\Set$ to the direct sum in $\Vect_F$, i.e., the coproduct in $\Vect_F$. Note that the tensor product $\otimes$ is \emph{not} the product in $\Vect_F$, as the dimension does not match: $\dim(F[X] \otimes F[Y]) = |X| \cdot |Y|$ while $\dim(F[X] \oplus F[Y]) = |X| + |Y|$. There is a unique but not isomorphic linear map $\phi : V \otimes W \to V \oplus W$ defined by $\phi(v \otimes w) = (v, w)$ for all $v \in V$ and $w \in W$.
\begin{remark}
...
Similarly, we have the following natural isomorphism in $\Vect_F$:
\[
  \Vect_F(V \otimes W, Z) \simeq \Vect_F(V, \Vect_F(W, Z))
\]
Note that $\Vect_F(W, Z)$ is a vector space over $F$, as $\Vect_F$ is just $\Hom_F$. Then, we have the following adjoint pair of endofunctors on $\Vect_F$:
\begin{center}
  \begin{tikzcd}[labels=auto]
    \Vect_F \arrow[r, "- \otimes X", yshift=0.5ex] & \Vect_F \arrow[l, "{\Hom_F(X, -)}", yshift=-0.5ex]
  \end{tikzcd}
\end{center}
...
\begin{definition}[Dual Space]\label{def:dual_space}
  Let $V$ be a linear space over $F$. The \emph{dual space} of $V$, denoted by $V^*$, is the linear space $\Hom_F(V, F)$, or $\Hom(V, F)$, consisting of all \hyperref[def:linear_functional]{linear functionals} from $V$ to $F$.
\end{definition}
...
\end{proposition}
\begin{proof}
  Without the loss of generality, we may assume $\dim(V) = n$ and $V = F^n$. Then $V^* = \Hom_F(F^n, F) \cong \Mat_{1 \times n}(F)$, the linear space of row matrices with $n$ entries. The linear space is the span of $n$ standard basis row matrices: $\hat{e}^1, \hat{e}^2, \ldots, \hat{e}^n$. So $\dim(V^*) = n = \dim(V)$.
\end{proof}
...
\begin{example}
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  Consider the cotangent vector $\mathsf{d}f_p$ at point $p$ for some smooth function $f : U \to \R$. It is a linear functional $\mathsf{d}f_p : \mathsf{T}_p U \to \R$ defined by $\mathsf{d}f_p(\vec{u}) = J_f(p) \vec{u}$ for all $\vec{u} \in \mathsf{T}_p U$. Here $\mathsf{T}_p U$ is the tangent space of $U$ at point $p$, which is a vector space over $\R$. Note that both $\vec{u}$ and $J_f(p)$ are depending on the choice of a coordinate system. However, $\mathsf{d}f_p$ is independent of any choice of coordinate system. In normal calculus, $\mathsf{d}f_p$ is called the \emph{first partial derivative} of $f$ at point $p$, and normally we write it as $f_x(p)$ and $f_y(p)$.
\end{example}
...
\begin{definition}[Bilinear Map]\label{def:bilinear_map}
  A map $B : U \times V \to W$ is called \emph{bilinear} if for all $u \in U$, the map $B(u, -) : V \to W$ is \hyperref[def:linear_map]{linear}, and for all $v \in V$, the map $B(-, v) : U \to W$ is linear.
\end{definition}

\begin{definition}[Natural Pairing]\label{def:pairing}
  Let $V$ be a linear space over $F$. A \emph{natural pairing}, or \emph{evaluation map} on $V$ is a \hyperref[def:bilinear_map]{bilinear map} $\langle-,-\rangle : V^* \times V \to F$ defined by $\langle f, u \rangle = f(u)$ where $f \in V^*$ and $u \in V$, i.e., a pairing between a \hyperref[def:linear_functional]{covector} and a vector.
\end{definition}

We have the following natural isomorphism between $V$ and $V^{**}$ induced by the natural pairing:
\begin{center}
  \begin{tikzcd}
    V^* \times V \arrow[d, <->, "\text{flip}"] \arrow[rr, "{\langle-,-\rangle}"] &[-2.4em] \arrow[d, Leftrightarrow] &[-2.4em] F \arrow[r, Leftrightarrow] & V^* \arrow[rr, "1_{V^*}"] &[-2.4em] \arrow[d, Leftrightarrow] &[-2.4em] \Hom_F(V, F) \\
    V \times V^* \arrow[rr] & \phantom{*} & F \arrow[r, Leftrightarrow]
    & V \arrow[rr, "\iota_V"] & \phantom{*} & \Hom_F(V^*, F)
  \end{tikzcd}
\end{center}
where $\iota_V : V \to V^{**}$ is defined by $\iota_V(u) = \check{u}$ such that $\check{u}(f) = f(u)$. Then $V^{**} = \Hom_F(V^*, F) \simeq V$.
...
\begin{definition}[Natural Transformation]\label{def:natural_transformation}
  Let $F, G : \Ca \to \D$ be two \hyperref[def:functor]{functors}. A \emph{natural transformation} $\eta : F \to G$ is a collection of \hyperref[def:morphism]{morphisms} $\eta_X : F(X) \to G(X)$ in $\D$ for all objects $X$ in $\Ca$, such that for all morphisms $f : X \to Y$ in $\Ca$, the following diagram commutes:
  \begin{center}
    \begin{tikzcd}[background color=ustblue!5]
...
\begin{definition}[Natural Isomorphism]\label{def:natural_isomorphism}
  A \emph{natural isomorphism}, or \emph{natural equivalence}, from \hyperref[def:functor]{functor} $F$ to functor $G$ is a \hyperref[def:natural_transformation]{natural transformation} $\eta : F \to G$ which has a two-sided inverse natural transformation $\eta^{-1} : G \to F$ such that $\eta \eta^{-1} = 1_G$ and $\eta^{-1} \eta = 1_F$. In this case, we say $F$ and $G$ are \emph{naturally isomorphic} or \emph{naturally equivalent}, denoted by $F \simeq G$.
\end{definition}
...
\begin{example}
  ...
  \[
    \Bil(U \times V, -) \simeq \Hom_F(U, \Hom_F(V, -))
  \]
  where both are endofunctors on $\Vect_F$. For any linear space $Z$ over $F$, we have the natural isomorphism:
  \[
    \musNatural : \Bil(U \times V, Z) \to \Hom_F(U, \Hom_F(V, Z))
  \]
\end{example}
...
\begin{enumerate}
  ...
  \item a bilinear map $f: V \times W \to Z$ naturally defines a linear map $f_{\musNatural} : V \to \Hom(W, Z)$;
  \item Show that the natural map $\musNatural : \Bil(V \times W, Z) \to \Hom(V, \Hom(W, Z))$ that sends $F$ to $F_{\musNatural}$ is a linear isomorphism. So we write \[ \Bil(V \times W, Z) \simeq \Hom(V, \Hom(W, Z)) \]
\end{enumerate}
...
\begin{problem}
Let $T : V_1 \to V_2$ be a linear map and $\B_i$ be a basis of linear space $V_i$. Denote by $\B_i^*$ the dual basis of $\B_i$ for vector space $V_i^*$, by $T^* : V_2^* \to V_1^*$ the dual map of $T$, i.e., the map that sends $f$ to $f \circ T$.
\begin{enumerate}
