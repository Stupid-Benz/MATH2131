...
\begin{example}
  In the category $\Vect_F$, the zero linear space $\{0\}$ is both an initial object and a terminal object, hence it is a \emph{zero object}.
\end{example}
...
\end{align*}
where $I$ is a finite index set. Hence, the product and coproduct coincide for finite collections of objects in $\Vect_F$, but not in infinite cases. Consider the following Figure~\ref{fig:product_coproduct} that illustrates the product and coproduct of an infinite collection of linear spaces ${\{ V_i \}}_{i \in I}$ over a field $F$.
...
\begin{figure}[!ht]
  ...
  \end{tikzpicture}
  \caption{The product and coproduct of an infinite collection of linear spaces ${\{ V_i \}}_{i \in I}$ over a field $F$. The left section $s_1$ represents an element in the product $\prod V_i$, and the right section $s_2$ represents an element in the coproduct $\bigoplus V_i$.}\label{fig:product_coproduct}
\end{figure}
...
Actually, the product and coproduct can be regarded as the generalisation of $\Map(X, F)$ and $F[X]$ respectively. We can consider the following Figure~\ref{fig:map_set_map} that illustrates the set map $s \colon X \to F$.
...
\begin{figure}[!ht]
  ...
  \end{tikzpicture}
  \caption{The set map $s \colon X \to F$ illustrated as sections over fibres.}\label{fig:map_set_map}
\end{figure}
...
\begin{remark}
  The universal property of free vector space over a set can be rephrased as follows: for any set $X$ and any linear space $V$ over $F$, there is a natural identification:
  \[
...
\begin{example}
  ...
  \]
  for all sets $Y$ and $Z$. This means that a set map $F \colon Y \times X \to Z$ corresponds to a set map $F_{\natural} \colon Y \to \Set(X, Z)$ such that a $y \in Y$ is mapped to a set map $F_{\natural}(y) \colon X \to Z$ defined by $F_{\natural}(y)(x) = F(y, x)$ for all $x \in X$.s
\end{example}
...
\begin{example}
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  Consider the cotangent vector $\mathsf{d}f_p$ at point $p$ for some smooth function $f \colon U \to \R$. It is a linear functional $\mathsf{d}f_p \colon \mathsf{T}_p U \to \R$ defined by $\mathsf{d}f_p(\vec{u}) = J_f(p) \vec{u}$ for all $\vec{u} \in \mathsf{T}_p U$. Here $\mathsf{T}_p U$ is the tangent space of $U$ at point $p$, which is a linear space over $\R$. Note that both $\vec{u}$ and $J_f(p)$ are depending on the choice of a coordinate system. However, $\mathsf{d}f_p$ is independent of any choice of coordinate system. In normal calculus, $\mathsf{d}f_p$ is called the \emph{first partial derivative} of $f$ at point $p$, and normally we write it as $f_x(p)$ and $f_y(p)$.
\end{example}
...
\begin{example}\label{ex:bilinear_natural_isomorphism}
  ...
  \]
  where $\Bil(V \times W, -)$ is the functor that sends a linear space $Z$ to the set of bilinear maps with the source $V \times W$. For any linear space $Z$ over $F$, we have the natural isomorphism:
  \[
    \natural \colon \Bil(V \times W, Z) \to \Hom(V, \Hom(W, Z))
  \]
\end{example}
...
\begin{enumerate}
  ...
  \item a bilinear map $f: V \times W \to Z$ naturally defines a linear map $f_{\natural} \colon V \to \Hom(W, Z)$;
  \item Show that the natural map $\natural \colon \Bil(V \times W, Z) \to \Hom(V, \Hom(W, Z))$ that sends $F$ to $F_{\natural}$ is a linear isomorphism. So we write \[ \Bil(V \times W, Z) \simeq \Hom(V, \Hom(W, Z)) \]
\end{enumerate}
...
Let $B_f$ be the matrix that represents $f_{\natural}$ with respect to the bases $\B_V$ and $\B_{W^*}$ via the commutative diagram
\begin{center}
  \begin{tikzcd}
    V \arrow[r, "f_{\natural}"] \arrow[d, "{[-]_{\B_V}}"] & W^* \arrow[d, "{[-]_{\B_{W^*}}}"] \\
    F^m \arrow[r, "B_f"] & (F^n)^*
  \end{tikzcd}
\end{center}
We say that a bilinear map $f: V \times W \to F$ is \emph{non-degenerate} if $f_{\natural}: V \to W^*$ is a linear isomorphism.
...
\begin{problem}
Let $T \colon V_1 \to V_2$ be a linear map and $\B_i$ be a basis of linear space $V_i$. Denote by $\B_i^*$ the dual basis of $\B_i$ for linear space $V_i^*$, by $T^* \colon V_2^* \to V_1^*$ the dual map of $T$, i.e., the map that sends $f$ to $f \circ T$.
\begin{enumerate}
