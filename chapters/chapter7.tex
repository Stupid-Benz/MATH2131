...
\begin{problem}
  Let $\omega$ be a bilinear form on $V$, i.e., a bilinear map $V \times V \to F$. Then $\omega_{\musNatural}: V \to V^*$ is the linear map tha sends $v$ to $\omega(v, -)$. Let $v_i$ be a basis of $V$ and the resulting dual basis of $V^*$ be denoted by $\hat{v}^i$. Let $A$ be the matrix representation of $\omega$ with respect to the basis $v_i$, i.e., $A = [\omega(v_i, v_j)]$. Let $A'$ be the matrix representation of $\omega_{\musNatural}$ with respect to the bases $v_i$ and $\hat{v}^i$. 
  \begin{enumerate}
