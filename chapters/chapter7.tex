...
\begin{enumerate}
	...
	\item Assume that the characteristic of the field $F$ is not 2, show that there is a natural linear equivalence $\Lambda^2 V^* \equiv {(\Lambda^2 V)}^*$.
\end{enumerate}
\end{problem}
...
\begin{problem}
  ...
  \begin{enumerate}
    ...
    \item The quotient map $V \otimes V \to S^2 V$ yields injective linear map ${(S^2 V)}^* \to {(V \otimes V)}^*$, that is not a surprise because any symmetric bilinear form is a bilinear form. Similarly, ${(\Lambda^2 V)}^*$ is a linear subsubspace of ${(V \otimes V)}^*$ as well. Show that $\GL(V)$ acts on ${(V \otimes V)}^*$ and leaves invariant both subspaces ${(S^2 V)}^*$ and ${(\Lambda^2 V)}^*$.
