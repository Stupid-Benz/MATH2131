...
\begin{definition}[Pseudo Inner Product]\label{def:pseudo_inner_product}
	A \emph{pseudo inner product} on a real linear space $V$ is a \emph{non-degenerate} symmetric bilinear form on $V$, i.e., an element $\langle-, -\rangle \in \mathcal{S}^2 V^*$ such that $\langle-, -\rangle_{\natural} : V \to V^*$ is an isomorphism.
\end{definition}
...
\begin{center}
	...
	\end{tikzcd}
\end{center}
Note that the restriction $\langle -, - \rangle$ is still an inner product on $W$. Also, the positive-definiteness of $\langle -, - \rangle$ implies that $\langle -, - \rangle$ is non-degenerate, i.e., the map $\langle -, - \rangle_{\natural} : W \to W^*$ is isomorphism. Note that $W$ and $W^*$ have the same dimension and it has a trivial kernel: $\langle u, - \rangle_W = 0$ implies $\langle u, u \rangle_W = 0$ implies $u = 0$. Now, suppose $w = (w_1, \cdots, w_k)$ is a basis of $W$ and $w^* = (w_1^*, \cdots, w_k^*)$ is the dual basis of $W^*$, then we have the following diagram:
\begin{center}
	\begin{tikzcd}
		0 \arrow[r] & \ker(\lambda_w) \arrow[r] & V \arrow[r, "\lambda_w"', two heads] & \R^k \arrow[r] \arrow[l, bend right, "s"'] & 0 \\
		& & W \arrow[u, hook] \arrow[r, <->, "{\langle -, - \rangle_{\natural}}"] & W^* \arrow[u, <->, "{[-]_{w^*}}"'] \\[-3.6em]
...
\begin{definition}[Hermitian Inner Products]\label{def:hermitian_inner_product}
	A \emph{Hermitian inner product}, or \emph{Hermitian product}, is a \emph{Hermitian form} that is also \emph{positive-definite}, i.e., for any $v \in V$, $\langle v, v \rangle \geq 0$ with equality if and only if $v = 0$. A Hermitian form on a complex linear space $V$ is a map $\langle -, - \rangle : V \times V \to \C$ such that the following properties hold:
...
\begin{remark}
	In physics, the Hermitian product is written with Dirac's bra-ket notation:
	\[
...
Then a complex linear space $V$ with an Hermitian product $\langle -, - \rangle$ is called a \emph{Hermitian space}. Also, the model / standard Hermitian space is $(\C^n, \langle -, - \rangle)$ with the standard Hermitian product, that is, the inner product defined above.
...
Similarly, there is only one Hermitian space, up to isomorphism, with dimension $n$, that is, $(\C^n, \langle -, - \rangle)$ with the standard Hermitian product, i.e., for any Hermitian space $V$ with dimension $n$, there exists a linear isomorphism between $V$ and $(\C^n, \langle -, - \rangle)$.
...
\subsection{Matrix representation of Hermitian products}

Then we have the matrix representation of Hermitian products as follows.

Let $V$ be a Hermitian space with Hermitian product $\langle -, - \rangle$. Then we can choose a basis $v = (v_1, v_2, \cdots, v_n)$ of $V$. Then we have
\[
	\omega = [\langle v_i, v_j \rangle]
\]
Note that $\omega$ is a Hermitian matrix, i.e., $\omega^* = \omega$. Then we claim that if $A$ and $\widetilde{A}$ are two matrix representations of the Hermitian product $\langle -, - \rangle$ with respect to two different bases $v$ and $\widetilde{v}$ respectively, then there exists an invertible matrix $P \in \GL_n(\C)$ such that:
\[
	\widetilde{A} = P^* A P
\]
...
Let $V$ be a Hermitian space with Hermitian product $\langle -, - \rangle$. We have the following equality for any linear operator $T : V \to V$:
\[
...
\end{proposition}
\begin{proof}
	We can reduce the problem to $\C^n$ and $\C^m$ with standard Hermitian products by choosing orthonormal bases of $V$ and $W$. Then we have $T$ represented by a matrix $A \in \Mat_{m \times n}(\C)$. Then we propose there is a matrix $B \in \Mat_{n \times m}(\C)$ such that for all $\vec{e}_i \in \C^n$ and $\vec{f}_j \in \C^m$, we have:
	\[
...
\section{Exercises}
\begin{problem}
Let $A$ be a real symmetric $n \times n$ matrix and $A_i$ be the matrix obtained from $A$ by deleting its last $n - i$ rows and last $n - i$ columns. Show that
\[
...
\begin{problem}
Let $A$ be a real symmetric $n \times n$ matrix. Assume that $A \geq 0$. It is clear that $A \geq 0 \iff A + tI > 0$ for any $t > 0$. Based on this observation and the result in the previous exercise to derive a necessary and sufficient condition for $A \geq 0$.
\end{problem}

\begin{problem}
Let $V$ be a $n$-dimensional linear space over a field $F$. The pairing $V^* \times V \to F$ yields the multilinear map $V^* \times V^* \times V \times V \to F$ that sends $(\alpha_1, \alpha_2, v_1, v_2)$ to $\alpha_1(v_1) \alpha_2(v_2)$. Thus we have a linear map
\[
	(V^* \otimes V^*) \otimes (V \otimes V) \to F
\]
or equivalently a linear map $\iota : V^* \otimes V^* \to (V \otimes V)^*$.
\begin{enumerate}
	\item Show that the linear map $\iota$ is a linear equivalence. In fact a natural one in the language of category. So we shall write $V^* \otimes V^* \equiv (V \otimes V)^*$.
	\item The quotient linear map $V \otimes V \to S^2 V$ yields the injective map $(S^2 V)^* \to (V \otimes V)^*$. So we have the following composition map
	      \begin{center}
		      \begin{tikzcd}
			      (S^2 V)^* \arrow[r, hook] & (V \otimes V)^* \arrow[r, "\equiv" description, phantom] & V^* \otimes V^* \arrow[r, two heads] & S^2 V^*
		      \end{tikzcd}
	      \end{center}
	      Show that this natural map $(S^2 V)^* \to S^2 V^*$ is a linear equivalence if and only if the characteristic of the field $F$ is not 2.
	\item Assume that the characteristic of the field $F$ is not 2, please find the inverse of the natural map in part (b).
	\item Assume that the characteristic of the field $F$ is not 2, show that there is a natural linear equivalence ${\bigwedge}^2 V^* \equiv ({\bigwedge}^2 V)^*$.
\end{enumerate}
\end{problem}

\begin{problem}
  Let $\omega$ be a bilinear form on $V$, i.e., a bilinear map $V \times V \to F$. Then $\omega_{\natural}: V \to V^*$ is the linear map tha sends $v$ to $\omega(v, -)$. Let $v_i$ be a basis of $V$ and the resulting dual basis of $V^*$ be denoted by $\hat{v}^i$. Let $A$ be the matrix representation of $\omega$ with respect to the basis $v_i$, i.e., $A = [\omega(v_i, v_j)]$. Let $A'$ be the matrix representation of $\omega_{\natural}$ with respect to the bases $v_i$ and $\hat{v}^i$. 
  \begin{enumerate}
    \item Show that $A'$ is the transpose of $A$. Thus $\omega$ is non-degenerate means that its any matrix representation is invertible.
    \item If $\tilde{v}_{\tilde{\imath}}$ is another basis of $V$, then there is a unique invertible matrix $S = [s^i_{\tilde{\imath}}]$ such that $\tilde{v}_{\tilde{\imath}} = v_i s^i_{\tilde{\imath}}$. Let $\tilde{A}$ be the matrix representation of $\omega$ with respect to the basis $\tilde{v}_{\tilde{\imath}}$. Show that $\tilde{A} = S^T A S$.
    \item Show that the map that sends $(A, S)$ to $S^T A S$ is a right action of $\GL_n(F)$ on $(V \otimes V)^*$.
    \item The quotient map $V \otimes V \to S^2 V$ yields injective linear map $(S^2 V)^* \to (V \otimes V)^*$, that is not a surprise because any symmetric bilinear form is a bilinear form. Similarly, $({\bigwedge}^2 V)^*$ is a linear subsubspace of $(V \otimes V)^*$ as well. Show that $\GL(V)$ acts on $(V \otimes V)^*$ and leaves invariant both subspaces $(S^2 V)^*$ and $({\bigwedge}^2 V)^*$.
    \item Show that, under the identification $(V \otimes V)^* \equiv V^* \otimes V^*$, we have 
    \[
        \omega = \omega(v_i, v_j) \hat{v}^i \otimes \hat{v}^j
    \]
    \item Assume that the characteristic of the field $F$ is not 2, then we have the natural identification $(S^2 V)^* \equiv S^2 V^*$ found in the last question. Show that, if $\omega$ is a symmetric bilinear form on $V$, then $\omega = \omega(v_i, v_j) \hat{v}^i \hat{v}^j$. Similarly, if $\omega$ is a skew-symmetric bilinear form on $V$, then $\omega = \omega(v_i, v_j) \hat{v}^i \wedge \hat{v}^j$.
  \end{enumerate}
\end{problem}