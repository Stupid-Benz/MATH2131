...
\begin{definition}[Linear Map]\label{def:linear_map}
  A \emph{linear map}, or \emph{linear transformation}, between two linear spaces $V$ and $W$ over the same \hyperref[def:field]{field} $F$ is a set map $T \colon V \to W$ that respects the \hyperref[def:linear_structure]{linear structure}; that is, for all $u, v \in V$ and all scalars $\alpha \in F$, the following properties hold:
  \begin{itemize}
    \item $T(u + v) = T(u) + T(v)$;
...
\begin{definition}[Kernel]\label{def:kernel}
  The \emph{kernel} of a \hyperref[def:linear_map]{linear map} $T \colon V \to W$ is the set of all vectors in $V$ that are mapped to the zero vector in $W$:
  \[
...
\begin{proposition}
  The kernel of a linear map $T \colon V \to W$ is a linear subspace of $V$.
\end{proposition}
\begin{proof}
...
\begin{definition}[Image]\label{def:image}
  The \emph{image} of a \hyperref[def:linear_map]{linear map} $T \colon V \to W$ is the set of all vectors in $W$ that can be expressed as $T(v)$ for some vector $v$ in $V$:
  \[
...
\begin{proposition}
  The image of a linear map $T \colon V \to W$ is a linear subspace of $W$.
\end{proposition}
\begin{proof}
...
\begin{definition}[Injective Linear Map]\label{def:injective_linear_map}
  A \hyperref[def:linear_map]{linear map} $T \colon V \to W$ is \emph{injective}, or a \emph{monomorphism}, if for any $u, v \in V$, $T(u) = T(v)$ implies that $u = v$.
\end{definition}

\begin{exercise}
  Show that a linear map $T \colon V \to W$ is injective if and only if $\ker(T) = \{ 0 \}$.
\end{exercise}

\begin{definition}[Surjective Linear Map]\label{def:surjective_linear_map}
  A \hyperref[def:linear_map]{linear map} $T \colon V \to W$ is \emph{surjective}, or an \emph{epimorphism}, if for any $w \in W$, there exists a vector $v \in V$ such that $T(v) = w$.
\end{definition}

\begin{exercise}
  Show that a linear map $T \colon V \to W$ is surjective if and only if $\im(T) = W$.
\end{exercise}

\begin{definition}[Linear Isomorphism]\label{def:isomorphism}
  A \hyperref[def:linear_map]{linear map} $T \colon V \to W$ is a \emph{linear isomorphism}, or an \emph{isomorphism}, if $T$ has an inverse map $T^{-1} \colon W \to V$ that is also a linear map, i.e., there exists a linear map $T^{-1} \colon W \to V$ such that $T^{-1} \circ T = \id_V$ and $T \circ T^{-1} = \id_W$. Then the linear spaces $V$ and $W$ are said to be \emph{isomorphic}, denoted by $V \cong W$.
\end{definition}
\begin{remark}
...
\begin{proposition}
  A linear map $T \colon V \to W$ is a linear isomorphism if and only if it is both injective and surjective.
\end{proposition}
\begin{proof}[listhack=true]
  \begin{description}[labelwidth=\widthof{($\Rightarrow$):~}]
    \item[($\Rightarrow$)] If $T$ is a linear isomorphism, then there exists a linear map $T^{-1} \colon W \to V$ such that $T^{-1} \circ T = \id_V$ and $T \circ T^{-1} = \id_W$.
          \begin{itemize}
...
\begin{example}
  The differential operator $D \colon F[x] \to F[x]$ is not an injective linear map as $D(1) = 0 = D(2)$ but it is a surjective linear map for $F$ is a field of characteristic 0.
\end{example}
...
\begin{definition}[Finite-Dimensional Linear Space]\label{def:finite_dimensional_linear_space}
  A linear space $V$ over $F$ is \emph{finite-dimensional} if there exists an \hyperref[def:isomorphism]{isomorphism} $T \colon V \to F^n$ for some positive integer $n$. The integer $n$ is the \emph{dimension} of the linear space $V$, denoted by $\dim_F(V)$ or simply $\dim(V)$.
\end{definition}
...
\begin{proposition}
  If there exists isomorphisms $T \colon V \to F^n$ and $S \colon V \to F^m$, then $n = m$.
\end{proposition}
\begin{proof}
...
\begin{center}
  \begin{tikzcd}
    V \arrow[r, <->, "T" description] \arrow[d, <->, "S" description] & F^n \\
    F^m \arrow[ru, <->, "TS^{-1}" description, sloped]
  \end{tikzcd}
\end{center}
...
\begin{exercise}\label{ex:characterise_sets_using_linear_maps}
  Let $V$ be an $n$-dimensional linear space, and $S = (v_1, \ldots, v_k)$ be an ordered set of $k$ vectors in $V$. Let $\phi_S \colon \F^k \to V$ be the linear map that sends $\vec{x} \in \F^k$ to $x^1 v_1 + \cdots + x^k v_k$. Show that
  \begin{enumerate}
...
\begin{definition}[Rank]\label{def:rank}
  The \emph{rank} of a \hyperref[def:linear_map]{linear map} $T \colon V \to W$ is the dimension of its \hyperref[def:image]{image}:
  \[
    \rank(T) = \dim(\im(T)).
  \]
\end{definition}
...
\begin{definition}[Nullity]\label{def:nullity}
  The \emph{nullity} of a \hyperref[def:linear_map]{linear map} $T \colon V \to W$ is the dimension of its \hyperref[def:kernel]{kernel}:
  \[
    \nullity(T) = \dim(\ker(T)).
  \]
\end{definition}
...
\begin{proposition}
  Every linear map $T \colon F^n \to F^m$ can be represented as multiplication by a unique $m \times n$ matrix $A$ over $F$. The matrix $A$ is called the \emph{standard matrix}, or the \emph{matrix representation}, of the linear map $T$. There is an isomorphism between two linear spaces $\Hom(F^n, F^m)$ and $\Mat_{m \times n}(F)$. Then we have
  \begin{itemize}
    ...
    \item For any matrix $A$ in $\Mat_{m \times n}(F)$, the corresponding linear map $T_A \colon F^n \to F^m$ is given by
          \[
...
\end{proposition}
\begin{proof}
  Let $T \colon F^n \to F^m$ be a linear map. Define the matrix $A$ as above. For any vector $\vec{x} \in F^n$, we can express $\vec{x}$ as a linear combination of $\vec{e}_1, \vec{e}_2, \ldots, \vec{e}_n$:
  \[
  ...
  \begin{align*}
    T\vec{x} & = T(x^1 \vec{e}_1 + x^2 \vec{e}_2 + \cdots + x^n \vec{e}_n) \\
             ...
             & = A \vec{x}.
  \end{align*}
  This shows that $T\vec{x}$ can be computed as the matrix-vector product $A\vec{x}$. Conversely, given a matrix $A$ in $\Mat_{m \times n}(F)$, we can define a linear map $T_A \colon F^n \to F^m$ by $T_A \vec{x} = A \vec{x}$. The linearity of $T_A$ follows from the properties of matrix multiplication.
\end{proof}
...
\begin{example}
  Consider the differential operator $D \colon F[x] \to F[x]$ defined by
  \[
...
\begin{center}
  \begin{tikzcd}
    F^n \arrow[r, "T", "A"'] \arrow[rr, bend left, "ST" description] \arrow[rr, bend right, "BA"' description] & F^m \arrow[r, "S", "B"'] & F^k
  \end{tikzcd}
\end{center}

\begin{proposition}
  The standard matrix of the composition $ST \colon F^n \to F^k$ is given by the matrix product $BA$, i.e., for any vector $\vec{x} \in F^n$, we have
...
\begin{proof}
  ...
  \begin{center}
    \begin{tikzcd}
      F^n \arrow[r, "A" description] \arrow[d, <->, "Q" description] & F^m \arrow[d, <->, "P" description] \\
      F^n \arrow[r, "N" description]                                   & F^m
    \end{tikzcd}
  \end{center}
...
Any linear map $T \colon V \to W$ between finite-dimensional linear spaces can be represented by a matrix once we choose bases for $V$ and $W$. The process of representing a linear map by a matrix is called \emph{trivialisation}. Consider the following commutative diagram:
\begin{center}
  \begin{tikzcd}
    V \arrow[r, "T" description] \arrow[d, <->, "{[-]_{\B_V}}" description] \arrow[dd, bend right=60, <->] & W \arrow[d, <->, "{[-]_{\B_W}}" description] \arrow[dd, bend left=60, <->] \\
    F^n \arrow[r, "A" description] \arrow[d, <->, "Q" description] & F^m \arrow[d, <->, "P" description] \\
    F^n \arrow[r, "N" description] & F^m
  \end{tikzcd}
\end{center}
...
\begin{definition}[Left Group Action]\label{def:group_action}
  A \emph{left group action} of a group $G$ on a set $X$ is a map $\cdot \colon G \times X \to X$ such that for all $g, h \in G$ and all $x \in X$, we have
  \begin{itemize}
...
\begin{remark}
  A right group action of a group $G$ on a set $X$ is defined similarly, with the action map $\cdot \colon X \times G \to X$ satisfying the compatibility condition $ (x \cdot g) \cdot h = x \cdot (gh)$ and the unital condition $x \cdot e = x$ for all $g, h \in G$ and all $x \in X$.
\end{remark}
...
\begin{center}
  \begin{tikzcd}
    W \times V \arrow[r, hook, "\iota \times \id_V" description] & V \times V \arrow[r, "+" description] & V
  \end{tikzcd}
\end{center}
...
\begin{definition}[Linear Quotient Map]\label{def:quotient_map}
  The \emph{linear quotient map} $\pi \colon V \to \quotient{V}{W}$ is the map that sends each vector $v \in V$ to its corresponding coset $v + W$ in the \hyperref[def:quotient_space]{quotient space} $\quotient{V}{W}$:
  \[
    \pi(v) = v + W = [v].
  \]
\end{definition}
...
\begin{proposition}
  There is a unique linear structure on the quotient space $\quotient{V}{W}$ such that the quotient map $\pi \colon V \to \quotient{V}{W}$ is a linear map.
\end{proposition}
\begin{proof}
...
If we want to visualise the graphical representation of the quotient space $\quotient{V}{W}$ and the quotient map $\pi \colon V \to \quotient{V}{W}$, we can refer to Figure~\ref{fig:quotient_map}.
\begin{figure}[ht!]
  \centering
  \begin{tikzpicture}
  ...
  \end{tikzpicture}
  \caption{A graphical representation of the quotient map $\pi \colon V \to \quotient{V}{W}$.}\label{fig:quotient_map}
\end{figure}
...
\begin{proof}
  ...
  \begin{center}
    \begin{tikzcd}
      ...
      F^n \arrow[u, <->, "{[-]_{\B_V}}" description] \arrow[ur, two heads, dashed, "\phi" description]
    \end{tikzcd}
  \end{center}
  ...
  \begin{center}
    \begin{tikzcd}
      W \arrow[r, hook, "\iota" description] & V \arrow[r, two heads, "\pi" description] & \quotient{V}{W} \\
      & F^n \arrow[u, <->, "{[-]_{\B_V}}" description] \arrow[ur, two heads, dashed, "\phi" description]
    \end{tikzcd}
  \end{center}
  Then we know that $\phi \colon F^n \to \quotient{V}{W}$ is a surjective linear map from a finite-dimensional linear space $F^n$ to $\quotient{V}{W}$. Thus, $\quotient{V}{W}$ is finitely generated.
\end{proof}
...
\begin{proposition}[Universal Property of Minimal Spanning Set]
  Let $S$ be a minimal spanning set of a linear space $V$. For any linear space $Z$ and any set map $\phi \colon S \to Z$, there exists a unique linear map $\widetilde{\phi} \colon V \to Z$ such that the following diagram commutes:
  \begin{center}
    \begin{tikzcd}
      S \arrow[r, hook, "\iota" description] \arrow[rd, "\phi" description] & V \arrow[d, dashed, "\widetilde{\phi}" description] \\
      & Z
    \end{tikzcd}
  \end{center}
\end{proposition}
...
This proposition shows that any set map from a minimal spanning set $S$ to another linear space $Z$ can be uniquely extended to a linear map from the entire space $V$, i.e., $\Map(S, Z) \cong \Hom(V, Z)$.

\begin{proposition}[Universal Property of Quotient Space]
  Let $W$ be a subspace of a linear space $V$. For any linear space $Z$ and any linear map $\phi \colon V \to Z$ such that $W \subseteq \ker(\phi)$, there exists a unique linear map $\widetilde{\phi} \colon \quotient{V}{W} \to Z$ such that the following diagram commutes:
  \begin{center}
    \begin{tikzcd}
      V \arrow[r, two heads, "\pi" description] \arrow[rd, "\phi" description] & \quotient{V}{W} \arrow[d, dashed, "\widetilde{\phi}" description] \\
      & Z
    \end{tikzcd}
  \end{center}
\end{proposition}
...
\begin{definition}[Cokernel]\label{def:cokernel}
  The \emph{cokernel} of a \hyperref[def:linear_map]{linear map} $T \colon V \to W$ is the \hyperref[def:quotient_space]{quotient space} of $W$ by the \hyperref[def:image]{image} of $T$:
  \[
...
\begin{definition}[Coimage]\label{def:coimage}
  The \emph{coimage} of a \hyperref[def:linear_map]{linear map} $T \colon V \to W$ is the \hyperref[def:quotient_space]{quotient space} of $V$ by the \hyperref[def:kernel]{kernel} of $T$:
  \[
...
\begin{center}
  \begin{tikzcd}
    \color{ustblue} V \arrow[rr, bend left, "T" description] & \color{ustblue} \ker(T) \arrow[l, ustblue, hook', "\iota" description] \arrow[r, "0" description] & W \\
    & \color{ustblue} Z \arrow[ul, ustblue, hook', "\phi" description] \arrow[u, ustblue, dashed, "\widetilde{\phi}" description] \arrow[ur, "0" description]
  \end{tikzcd}
  \begin{tikzcd}
    V \arrow[rr, bend left, "T" description] \arrow[r, "0" description] \arrow[dr, "0" description] & \color{ustblue} \coker(T) \arrow[d, ustblue, dashed, "\widetilde{\phi}" description] & W \arrow[l, ustblue, two heads, "\pi" description] \arrow[dl, ustblue, "\phi" description] \\
    & \color{ustblue} Z &
  \end{tikzcd}
\end{center}
...
\begin{definition}[Exact Sequence]\label{def:exact_sequence}
  ...
  \begin{center}
    \begin{tikzcd}[background color=ustblue!5]
      \cdots \arrow[r] & V_{i-1} \arrow[r, "f_{i-1}" description] & V_i \arrow[r, "f_i" description] & V_{i+1} \arrow[r] & \cdots
    \end{tikzcd}
  \end{center}
...
\begin{example}
  ...
  \begin{center}
    \begin{tikzcd}
      0 \arrow[r] & V_1 \arrow[r, "\iota_1" description] & V \arrow [r, "\pi_2" description] & V_2 \arrow[r] & 0
    \end{tikzcd}
  \end{center}
  ...
  \begin{itemize}
    \item $\pi_2$ has a right inverse, or a section, i.e., there exists a linear map $\iota_2 \colon V_2 \to V$ such that $\pi_2 \circ \iota_2 = \id_{V_2}$. This is because $V_2$ has a minimal spanning set. Thus, for each element in the minimal spanning set of $V_2$, we can choose one representative in $V$ and define the map on the minimal spanning set. Then we can extend it to the whole space.

    \item $\iota_1$ has a left inverse, or a retraction, i.e., there exists a linear map $\pi_1 \colon V \to V_1$ such that $\pi_1 \circ \iota_1 = \id_{V_1}$. This is because $\iota_1$ is injective. Thus, for each element in $V_1$, we can choose one representative in $V$ and define the map on the whole space by sending all other elements to zero.
  \end{itemize}

  The exact sequence becomes:
  \begin{center}
    \begin{tikzcd}
      0 \arrow[r] & V_1 \arrow[r, ustgold, hook, "\iota_1" description] & V \arrow[r, ustblue, two heads, "\pi_2" description] \arrow[l, ustblue, two heads, "\pi_1" description, bend right] & V_2 \arrow[r] \arrow[l, ustgold, hook', "\iota_2" description, bend left] & 0
    \end{tikzcd}
  \end{center}
\end{example}
...
\begin{proof}
  ...
  Note that all $V$, $V_1$ and $V_2$ are finite-dimensional. Then $V_2$ has a minimal spanning set, let say $S$. Then we construct $\iota_2 \colon s \mapsto \iota_2(s)$ where $\iota_2(s)$ is a choice of element from $\pi_2^{-1}(s) \neq \emptyset$ for each $s \in S$. Then we extend it to the whole space linearly. Thus, $\iota_2$ is injective.
...
\begin{exercise}[Dimension Formula]
  ...
  \begin{center}
    \begin{tikzcd}
      0 \arrow[r] & V_1 \cap V_2 \arrow[r, hook, "\iota" description] & V_1 \arrow[r, two heads, "\pi" description] & \quotient{(V_1 + V_2)}{V_2} \arrow[r] & 0
    \end{tikzcd}
  \end{center}
...
\begin{theorem}[Rank-Nullity Theorem]
  For a \hyperref[def:linear_map]{linear map} $T \colon V \to W$ between \hyperref[def:finite_dimensional_linear_space]{finite-dimensional linear spaces}, we have:
  \begin{equation}
    \dim(V) = \rank(T) + \nullity(T).
  \end{equation}
\end{theorem}
\begin{proof}
  ...
  \begin{center}
    \begin{tikzcd}[column sep=huge]
      0 \arrow[r] & \ker(T) \arrow[r, hook, "\iota" description] & V \arrow[r, two heads, "T" description] & \im(T) \arrow[r] & 0
    \end{tikzcd}
  \end{center}
...
\begin{proof}
  ...
  \begin{center}
    \begin{tikzcd}[column sep=huge]
      0 \arrow[r] & \im(T) \arrow[r, hook, "\iota" description] & W \arrow[r, two heads, "\pi" description] & \coker(T) \arrow[r] & 0
    \end{tikzcd}
  \end{center}
...
\begin{proof}
  ...
  \begin{center}
    \begin{tikzcd}[column sep=huge]
      0 \arrow[r] & \ker(T) \arrow[r, hook, "\iota" description] & V \arrow[r, two heads, "\pi" description] & \coim(T) \arrow[r] & 0
    \end{tikzcd}
  \end{center}
...
We have already known that any linear map $T \colon V \to W$ between finite-dimensional linear spaces can be represented by a matrix once we choose bases for $V$ and $W$. Moreover, we can choose appropriate coordinate maps such that the matrix representation of $T$ is in canonical form. How about the abstract form of the linear map without choosing any bases or coordinate maps? We can see it using exact sequences. Consider the following commutative diagram:
\begin{center}
  \begin{tikzcd}
    0 \arrow[d] & 0 \\
    \ker(T) \arrow[d, hook] & \coker(T) \arrow[u] \arrow[d, bend left, hook, "s_2" description, ustred] \\
    V \arrow[d, two heads] \arrow[r, "T" description] \arrow[dr, ustblue, two heads, "\overline{T}" description] & W \arrow[u, two heads] \\
    \coim(T) \arrow[d] \arrow[r, <->, "T'" description] \arrow[u, bend left, hook, "s_1" description, ustred] & \im(T) \arrow[u, hook] \\
    0 & 0 \arrow[u]
  \end{tikzcd}
\end{center}
Here, each column is a short exact sequence and the square in the middle commutes. Also, $\overline{T}$ and $T'$ are isomorphisms. Moreover, we have the sections $s_1$ and $s_2$ that are the right inverses of the projections from $V$ to $\coim(T)$ and from $W$ to $\coker(T)$ respectively. Thus, we can decompose $V$ and $W$ into $V = \im(s_1) \oplus \ker(T)$ and $W = \im(s_2) \oplus \im(T)$ respectively. Then, with respect to these decompositions, the linear map $T \colon V \to W$ can be represented as:
\begin{center}
  \begin{tikzpicture}
    \matrix (m) [matrix of math nodes, column sep=4.8em, minimum width=2em]
    {\im(s_1) \oplus \ker(T) & \im{T} \oplus \im(s_2) \\};
    \path[arrow] (m-1-1) edge node[above] {$T = \begin{bmatrix}
            \widetilde{T} & 0 \\
            0             & 0
          \end{bmatrix}$} (m-1-2);
  \end{tikzpicture}
\end{center}
where $\widetilde{T} \colon \im(s_1) \to \im(T)$ is an isomorphism, as there are isomorphisms $T' \colon \coim(T) \to \im(T)$ and $\im(s_1) \cong \coim(T)$. Then the graph below commutes:
\begin{center}
  \begin{tikzcd}
    \im(s_1) \arrow[r, <->, dashed, "\widetilde{T}" description] & \im(T) \\
    \coim(T) \arrow[u, <->, "s_1" description] \arrow[ur, <->, "T'" description]
  \end{tikzcd}
\end{center}
...
\begin{enumerate}
  ...
  \item any linear map $\phi \colon {(F^m)}^* \to {(F^n)}^*$ is of the form $\phi(\alpha) = \alpha A$ for a unique matrix $A$;
...
\begin{problem}
Let $f \colon V \to W$ be a linear map. Show that
\begin{enumerate}
  ...
  \item $f$ is surjective $\iff$ for any linear map $g \colon W \to Z$, $gf = 0 \implies g = 0$;
  \item $f$ is injective $\iff$ for any linear map $h \colon U \to V$, $fh = 0 \implies h = 0$.
\end{enumerate}
...
\begin{problem}
Let $f \colon V \to W$ be a set map between linear spaces. Show that
\begin{enumerate}
...
\begin{problem}
We say a linear map $f \colon V \to W$ is \emph{imbedding} if the map $\overline{f} \colon V \to \im(f)$ that sends $v$ to $f(v)$ is a linear isomorphism. Show that $f$ is imbedding $\iff$ $f$ is one-to-one.

An optional exercise: We say a topological map, i.e., continuous map, $f \colon X \to Y$ is imbedding if the map $\overline{f} \colon X \to \im f$ that sends $x$ to $f(x)$ is a topological equivalence, i.e., homeomorphism. Show that $f$ is imbedding implies that $f$ is one-to-one, but the converse is not true.
\end{problem}
...
\begin{enumerate}
  \item Show that there is a unique linear structure on $\quotient{V}{W}$ such that the quotient map $q \colon V \to \quotient{V}{W}$ is a linear map.
  \item Show that, for any linear map $\phi \colon V \to Z$ such that $\phi(v) = 0$ for any $v \in W$, there \emph{is} a \emph{unique} linear map $\overline{\phi} \colon \quotient{V}{W} \to Z$ such that \[ \overline{\phi} \circ q = \phi. \]
\end{enumerate}
...
\begin{enumerate}[resume]
  \item Let $W$ be a linear subspace of $V$. Then the inclusion map \begin{tikzcd}[cramped] W \arrow[r, "\iota" description] & V \end{tikzcd} is a linear map with image inside $V$. Please formulate and prove the universal property for the inclusion map $\iota$.
\end{enumerate}
\end{problem}
...
\begin{enumerate}
  ...
  \item the splitting in part (c) defines two maps, one is from $V$ to $V_1$ and is denoted by $\pi_1$ and the other is $\pi_2 \colon V \to V_2$;
  \item $\pi_1$ is linear and the sequence
        \begin{center}
          \begin{tikzcd}
            0 & V_1 \arrow[l] & V \arrow[l, "\pi_1" description] & V_2 \arrow[l, "\iota_2" description] & 0 \arrow[l]
          \end{tikzcd}
        \end{center}
        is exact;
...
\begin{problem}
Let $A$ be an $m \times n$-matrix, then the multiplication by $A$ defines a linear map $f \colon \F^n \to \F^m$. The rank of $A$, denoted by $\rank{A}$, is defined to be the rank of the linear map $f$. Note: $\im(f) = \col(A)$ --- the span of columns of $A$.
\begin{enumerate}
