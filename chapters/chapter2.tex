...
\begin{definition}[Finite-Dimensional Linear Space]\label{def:finite_dimensional_linear_space}
	A linear space $V$ over $F$ is \emph{finite-dimensional} if there exists an \hyperref[def:isomorphism]{isomorphism} $T : V \to F^n$ for some positive integer $n$. The integer $n$ is the \emph{dimension} of the linear space $V$, denoted by $\dim_F(V)$ or simply $\dim(V)$.
\end{definition}
...
\begin{figure}[ht!]
	...
	\end{tikzpicture}
	\caption{A group action of $\SO(2)$ on the plane.}
	\label{fig:group_action}
\end{figure}
...
\begin{figure}[ht!]
	...
	\begin{tikzpicture}
		...
		\draw[thick,red] (-2,-1) node [left] {$[v'_1] = [v_1] = v_1 + W$} -- (1,2);
	...
	\end{tikzpicture}
	\caption{A graphical representation of the quotient space $\quotient{V}{W}$.}
...
\begin{figure}[ht!]
	...
	\end{tikzpicture}
	\caption{A graphical representation of the quotient map $\pi : V \to \quotient{V}{W}$.}
	\label{fig:quotient_map}
\end{figure}
...
\begin{center}
	\begin{tikzcd}
		V \arrow[rr, bend left, "T" description] & \ker(T) \arrow[l, hook', "\iota" description] \arrow[r, "0" description] & W \\
		& Z \arrow[ul, hook', "\phi" description] \arrow[u, dashed, "\widetilde{\phi}" description] \arrow[ur, "0" description]
	\end{tikzcd}
	\begin{tikzcd}
		V \arrow[rr, bend left, "T" description] \arrow[r, "0" description] \arrow[dr, "0" description] & \coker(T) \arrow[d, dashed, "\widetilde{\phi}" description] & W \arrow[l, two heads, "\pi" description] \arrow[dl, "\phi" description] \\
		& Z &
	\end{tikzcd}
\end{center}
...
\begin{figure}[ht!]
	...
	\begin{tikzpicture}
		...
		\draw[blue, -Stealth] (0,0) -- (-2.5,0) node [midway, above] {\scriptsize $\ker(\iota_1)$};
		\draw[red, -Stealth] (-2.5,0.75) -- (0,1) node [midway, above] {\scriptsize $\im(\iota_1)$};
		\draw[red, -Stealth] (-2.5,-0.75) -- (0,-1) node [midway, below] {\scriptsize $\im(\iota_1)$};
		\draw[blue, -Stealth] (2.5,0) -- (0,1) node [midway, above] {\scriptsize $\ker(\pi_2)$};
		\draw[blue, -Stealth] (2.5,0) -- (0,-1) node [midway, below] {\scriptsize $\ker(\pi_2)$};
		\draw[red, -Stealth] (0,1.5) -- (2.5,1) node [midway, above] {\scriptsize $\im(\pi_2)$};
		\draw[red, -Stealth] (0,-1.5) -- (2.5,-1) node [midway, below] {\scriptsize $\im(\pi_2)$};
	...
	\end{tikzpicture}
	\caption{An Euler diagram illustrating a short exact sequence of linear spaces.}
	\label{fig:short_exact_sequence_diagram}
\end{figure}
...
\begin{proposition}
	The linear space $V$ is isomorphic to the direct sum of the images of $\iota_1$ and $\iota_2$:
	\[
...
\begin{exercise}[Dimension Formula]
	...
	\[
		\quotient{V_1}{(V_1 \cap V_2)} \cong \quotient{(V_1 + V_2)}{V_2},
	\]
...
\begin{proof}
	...
	\begin{center}
		...
		\end{tikzcd}
	\end{center}
	Then we have $V = \ker(T) \oplus \im(T)$. Thus, we have $\dim(V) = \dim(\ker(T)) + \dim(\im(T))$. This shows that $\rank(T) + \nullity(T) = \dim(V)$.
\end{proof}

\begin{remark}
	Note that the direct sum decomposition $V = \ker(T) \oplus \im(T)$ in the proof is an internal direct sum decomposition.
\end{remark}
...
\begin{proof}
	...
	\begin{center}
		...
		\end{tikzcd}
	\end{center}
	Then we have $W \cong \im(T) \oplus \coker(T)$. Thus, we have $\dim(W) = \dim(\im(T)) + \dim(\coker(T))$. This shows that $\rank(T) + \dim(\coker(T)) = \dim(W)$.
\end{proof}

\begin{remark}
	However, unlike the case of kernel and image, the direct sum decomposition $W \cong \im(T) \oplus \coker(T)$ in the proof is an external direct sum decomposition. The difference is that $\im(T)$ is a subspace of $W$ while $\coker(T)$ is a quotient space of $W$. Thus, we cannot say that $W$ is the internal direct sum of $\im(T)$ and $\coker(T)$. Internal direct sum requires both components to be subspaces of the same space.
\end{remark}
...
\begin{center}
	\begin{tikzcd}
		0 \arrow[d] & 0 \\
		\ker(T) \arrow[d, hook] & \coker(T) \arrow[u] \arrow[d, bend left, hook, "s_2" description, red] \\
		V \arrow[d, two heads] \arrow[r, "T" description] \arrow[dr, ustblue, two heads, "\overline{T}" description] & W \arrow[u, two heads] \\
		\coim(T) \arrow[d] \arrow[r, <->, "T'" description] \arrow[u, bend left, hook, "s_1" description, red] & \im(T) \arrow[u, hook] \\
		0 & 0 \arrow[u]
	\end{tikzcd}
\end{center}
Here, each column is a short exact sequence and the square in the middle commutes. Also, $\overline{T}$ and $T'$ are isomorphisms. Moreover, we have the sections $s_1$ and $s_2$ that are the right inverses of the projections from $V$ to $\coim(T)$ and from $W$ to $\coker(T)$ respectively. Thus, we can decompose $V$ and $W$ into $V = \im(s_1) \oplus \ker(T)$ and $W = \im(s_2) \oplus \im(T)$ respectively. Note that they are internal direct sum decompositions. Then, with respect to these decompositions, the linear map $T : V \to W$ can be represented as:
\begin{center}
	\begin{tikzpicture}
		\matrix (m) [matrix of math nodes, column sep=4.8em, minimum width=2em]
		{\im(s_1) \oplus \ker(T) & \im{T} \oplus \im(s_2) \\};
		\path[->] (m-1-1) edge node[above] {$T = \begin{bmatrix}
						\widetilde{T} & 0 \\
						0             & 0
					\end{bmatrix}$} (m-1-2);
	\end{tikzpicture}
...
\begin{enumerate}
	...
	\item in case $f$ is linear, its domain is naturally linear equivalent to its graph: domain $f = \Gamma_f$.
\end{enumerate}
\end{problem}

\begin{problem}
We say a linear map $f: V \to W$ is \emph{imbedding} if the map $\overline{f} : V \to \im(f)$ that sends $v$ to $f(v)$ is a linear equivalence. Show that $f$ is imbedding $\iff$ $f$ is one-to-one.
