...
\section{Linear Maps, Kernel, and Image}

\subsection{Linear Maps}
We begin by defining linear maps between linear spaces.
...
\begin{proof}[listhack=true]
  \begin{description}[labelwidth=\widthof{\bfseries ($\Rightarrow$):~}]
    \item[($\Rightarrow$)] If $T$ is a linear isomorphism, then there exists a linear map $T^{-1} \colon W \to V$ such that $T^{-1} \circ T = \id_V$ and $T \circ T^{-1} = \id_W$.
          \begin{description}[labelwidth=\widthof{\bfseries Surjective:~}]
            \item[Injective] $T(u) = T(v) \implies T^{-1}(T(u)) = T^{-1}(T(v)) \implies u = v$ for any $u, v \in V$.
            \item[Surjective] For any $w \in W$, let $v = T^{-1}(w)$. Then we have $T(v) = T(T^{-1}(w)) = w$.
          \end{description}
    ...
    \item [($\Leftarrow$)] If $T$ is both injective and surjective, we can define the inverse map $T^{-1} \colon W \to V$ as follows: for any $w \in W$, since $T$ is surjective, there exists a vector $v \in V$ such that $T(v) = w$. We define $T^{-1}(w) = v$. To show that $T^{-1}$ is well-defined, suppose there are two vectors $v_1, v_2 \in V$ such that $T(v_1) = w$ and $T(v_2) = w$. Then we have $T(v_1) = T(v_2)$, which implies that $v_1 = v_2$ since $T$ is injective. Therefore, $T^{-1}$ is well-defined.
          \begin{description}[labelwidth=\widthof{\bfseries Invertible:~}]
            \item[Invertible] for all $v \in V$, $w \in W$, we have $T^{-1}(T(v)) = v$ and $T(T^{-1}(w)) = w$.
            \item[Linearity] For any $w_1, w_2 \in W$ and any scalars $\alpha, \beta \in F$, let $v_1 = T^{-1}(w_1)$ and $v_2 = T^{-1}(w_2)$. Then we have
                  \begin{align*}
                    T^{-1}(\alpha \cdot w_1 + \beta \cdot w_2) & = T^{-1}(\alpha \cdot T(v_1) + \beta \cdot T(v_2))             \\
                                                               & = T^{-1}(T(\alpha \cdot v_1 + \beta \cdot v_2))                \\
                                                               & = \alpha \cdot v_1 + \beta \cdot v_2                           \\
                                                               & = \alpha \cdot T^{-1}(w_1) + \beta \cdot T^{-1}(w_2). \qedhere
                  \end{align*}
          \end{description}
  \end{description}
\end{proof}
...
\begin{definition}[Finite-Dimensional Linear Space]\label{def:finite_dimensional_linear_space}
  A linear space $V$ over $F$ is \emph{finite-dimensional} if there exists an \hyperref[def:linear_isomorphism]{isomorphism} $T \colon V \to F^n$ for some positive integer $n$. The integer $n$ is the \emph{dimension} of the linear space $V$, denoted by $\dim_F(V)$ or simply $\dim(V)$.
\end{definition}
...
\subsection{Elementary Row Operations and Elementary Column Operations}
...
\subsection{Canonical Forms of Matrices and Trivialisation}
...
\end{proposition}
\begin{proof}[listhack=true]
  \begin{description}[labelwidth=\widthof{\bfseries (1) $\implies$ (2):~}]
...
\section{Universal Properties in Linear Spaces}
...
\begin{example}
  ...
  \begin{center}
    \begin{tikzcd}
      0 \arrow[r] & V_1 \arrow[r, ustgold, hook, "\iota_1"] & V \arrow[r, ustblue, two heads, "\pi_2"] \arrow[l, ustblue, two heads, "\pi_1", bend left] & V_2 \arrow[r] \arrow[l, ustgold, hook', "\iota_2", bend right] & 0
    \end{tikzcd}
  \end{center}
\end{example}
...
\begin{figure}[ht!]
  ...
  \begin{tikzpicture}
    ...
    \draw[ustred, -Stealth] (-5,0) -- (-2.5,0) node [midway, above] {\scriptsize $\im(0)$};
    \draw[ustblue, -Stealth] (0,0) -- (-2.5,0) node [midway, above] {\scriptsize $\ker(\iota_1)$};
    \draw[ustred, -Stealth] (-2.5,0.75) -- (0,1) node [midway, above] {\scriptsize $\im(\iota_1)$};
    \draw[ustred, -Stealth] (-2.5,-0.75) -- (0,-1) node [midway, below] {\scriptsize $\im(\iota_1)$};
    \draw[ustblue, -Stealth] (2.5,0) -- (0,1) node [midway, above] {\scriptsize $\ker(\pi_2)$};
    \draw[ustblue, -Stealth] (2.5,0) -- (0,-1) node [midway, below] {\scriptsize $\ker(\pi_2)$};
    \draw[ustred, -Stealth] (0,1.5) -- (2.5,1) node [midway, above] {\scriptsize $\im(\pi_2)$};
    \draw[ustred, -Stealth] (0,-1.5) -- (2.5,-1) node [midway, below] {\scriptsize $\im(\pi_2)$};
    \draw[ustblue, -Stealth] (5,0) -- (2.5,1) node [midway, above] {\scriptsize $\ker(0)$};
    \draw[ustblue, -Stealth] (5,0) -- (2.5,-1) node [midway, below] {\scriptsize $\ker(0)$};
...
\begin{center}
  \begin{tikzcd}
    0 \arrow[d] & 0 \\
    \ker(T) \arrow[d, hook] & \coker(T) \arrow[u] \arrow[d, bend left=60, hook, "s_2", ustred] \\
    V \arrow[d, two heads] \arrow[r, "T"] \arrow[dr, ustblue, two heads, "\overline{T}"] & W \arrow[u, two heads] \\
    \coim(T) \arrow[d] \arrow[r, <->, "T'"] \arrow[u, bend left=60, hook, "s_1", ustred] & \im(T) \arrow[u, hook] \\
    0 & 0 \arrow[u]
  \end{tikzcd}
\end{center}
...
\end{enumerate}
Remark: $\quotient{V}{W}$ is called the quotient space of $V$ by the subspace $W$ and is also called the algebraic normal space of $V$ in $W$. It is a fact that $\dim(\quotient{V}{W}) = \dim(V) - \dim(W)$.
\begin{enumerate}[resume]
