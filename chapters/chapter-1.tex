...
\begin{proof}
	Let $e_1$ and $e_2$ be two identity elements for the operation $\cdot$.
	Then, for any element $x \in S$, we have:
	\begin{gather*}
...
\begin{example}
	...
	Elements in $\F^n$ are written as $\vec x$ and are called \emph{column vectors}\index{column vectors}. $\F^n$ is a linear space over $\F$ with the following operations defined entrywisely:
	\begin{align*}
		+ : \F^n \times \F^n &\to \F^n \\
