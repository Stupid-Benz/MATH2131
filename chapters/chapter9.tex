...
\end{figure}
The red line is the smallest affine subspace containing $p_0$ and $p_1$, i.e., the affine span of $p_0$ and $p_1$. We may write $\spn\{ p_0, p_1 \} := \{ c^0 p_0 + c^1 p_1 \mid c^0 + c^1 = 1, c^i \in \R \} = \{ tp_0 + (1 - t) p_1 \mid t \in \R \}$. Note that $\overline{p_0 p_1} = \{ tp_0 + (1 - t) p_1 \mid t \in [0, 1] \}$ is a subset of the affine span.
