...
\usetikzlibrary{matrix, positioning, arrows.meta, decorations.pathreplacing} % <-- Add this line to enable matrix of math nodes
...
\newcommand{\M}[2]{\text{M}_{#1}(#2)} % Matrix set notation
\newcommand{\Map}{\text{Map}} % Map notation
\newcommand{\rank}[1]{\text{Rank}(#1)} % Rank of a matrix
\newcommand{\coker}[1]{\text{Coker}(#1)} % Cokernel of a linear map
\newcommand{\coim}[1]{\text{Coim}(#1)} % Coimage of a linear map
\newcommand{\nullity}[1]{\text{Nullity}(#1)} % Nullity of a linear map
\newcommand{\id}{\text{id}} % Identity map notation
\newcommand{\nul}[1]{\text{Nul}(#1)} % Null space of a linear map
\newcommand{\col}[1]{\text{Col}(#1)} % Column space of a matrix
\newcommand{\C}{\mathcal{C}} % Category notation
\newcommand{\Set}{\textbf{Set}} % Set category notation
\newcommand{\Vect}{\textbf{Vec}} % Vector space category notation
\newcommand{\Mor}{\text{Mor}} % Morphism set notation
\newcommand{\Ob}[1]{\text{Ob}(#1)} % Object set notation
...
% 
% \noindent \textsc{Published by Hong Kong University of Science and Technology}\\ % Publisher
% 
...
\begin{center}
    \begin{tikzcd}
        V \arrow[d, hook, two heads] \arrow[dd, bend right, hook, two heads] \arrow[r, "T", hook, two heads] & W \arrow[d, hook, two heads] \arrow[dd, bend left, hook, two heads] \\
        \F^n \arrow[d, hook, two heads] \arrow[r, "A", hook, two heads] & \F^m \arrow[d, hook, two heads] \\
        \F^n \arrow[r, "B", hook, two heads] & \F^m
    \end{tikzcd}
\end{center}
...
\begin{center}
    \begin{tikzcd}[row sep=tiny, column sep=tiny]
        ...
        \Map(S, Z) \arrow[r, leftrightarrow, "\cong" description] & \text{Hom}(V, Z) \\
...
\begin{center}
    \begin{tikzcd}
        ...
        V_1 \arrow[d, two heads] \arrow[r, "f"] \arrow[dr, ocre, "f", two heads] & V_2 \arrow[u, two heads] \\
...
\end{proposition}
\begin{proof}
    Without the loss of generality, we may assume $\dim{V} = n$ and $V = \F^n$. Then $V^* = \Hom_{\F}(\F^n, \F) \equiv \M{1 \times n}{\F}$, the linear space of row matrices with $n$ entries. The linear space is the span of $n$ standard basis row matrices: $\hat{e_1}, \hat{e_2}, \ldots, \hat{e_n}$. So $\dim(V^*) = n = \dim(V)$. We can say $V^* \cong V$.
\end{proof}

We have a map $\phi_s : \F^n \to (\F^n)^* \supset S = \{\hat{e_1}, \hat{e_2}, \ldots, \hat{e_n}\}$ defined by $\phi_s(\vec{x}) = \sum_{i=1}^n x_i \hat{e_i}$. This is a vector space isomorphism but not a natural isomorphism, as it depends on the choice of $S$.
...
\begin{proposition}
    ...
    \[
        \begin{split}
            \B_V &\equiv \B_{V^*} \\
            \vec{v} = (v_1, v_2, \ldots, v_n) &\equiv (\hat{v_1}, \hat{v_2}, \ldots, \hat{v_n}) = \vec{v}^* \\
        \end{split}
    \]
...
\begin{center}
    ...
    \end{tikzcd}
\end{center}
The projection map $\pi_i$ is a linear functional in $(\F^n)^*$ that sends $\vec{x} = (\vec{x_1}, \vec{x_2}, \ldots, \vec{x_n})$ to $x_i$. It is actually $\hat{e_i}$. Note that $[-]_V : V \to \F^n$ is a coordinate map defined by a basis $\vec{v} = (v_1, v_2, \ldots, v_n) \in \B_V$ such that $[v_j]_V = \hat{e_j}$ for all $1 \leq j \leq n$. It is a unique linear map which identify $v_i$ with $\hat{e_i}$. It can be done by trivialisation of $V$ with respect to the basis $\vec{v}$. Then we define $\hat{v_i}(v_j) = \delta_{ij}$ for all $1 \leq i, j \leq n$.

Then we have to consider whether $(\hat{v_1}, \hat{v_2}, \ldots, \hat{v_n})$ is a basis of $V^*$. As $\dim{V^*} = n$, we only need to show that $(\hat{v_1}, \hat{v_2}, \ldots, \hat{v_n})$ is a spanning set of $V^*$ or linearly independent. We have to check whether the equation $\sum_{i=1}^n x_i \hat{v_i} = 0$ for some $x_i \in \F$ has only the trivial solution. Applying it to $v_j$ for all $1 \leq j \leq n$, we have $0 = \sum_{i=1}^n x_i \hat{v_i}(v_j) = \sum_{i=1}^n x_i \delta_{ij} = x_j$. So $x_j = 0$ for all $1 \leq j \leq n$. This means that $(\hat{v_1}, \hat{v_2}, \ldots, \hat{v_n})$ is linearly independent, and hence it is a basis of $V^*$. We call it the \emph{dual basis} of the basis $\vec{v} = (v_1, v_2, \ldots, v_n)$ and denote it by $\vec{v}^* = (\hat{v_1}, \hat{v_2}, \ldots, \hat{v_n})$. 

Then we have to show that there is a unique basis in $V^*$ that satisfies $\hat{v_i}(v_j) = \delta_{ij}$. Let $V = \F^n$ and $\vec{v} = (\vec{v_1}, \vec{v_2}, \ldots, \vec{v_n})$ be a basis of $V$. Then $A = [\vec{v_1}, \vec{v_2}, \ldots, \vec{v_n}]$ is an invertible matrix. Let $(\alpha_1, \alpha_2, \ldots, \alpha_n)$ be a basis of $V^*$. Then we have the following equations:
\[
    ...
    \end{bmatrix} = I_n
\]
Then $(\alpha_1, \alpha_2, \ldots, \alpha_n) = A^{-1}$. So the dual basis is unique.
...
% Dual space functors and identity functors such stuffs. Later ask.

However, $V$ is naturally isomorphic to its double dual space $V^{**} = (V^*)^*$. 
...
\begin{example}
    Consider the following open subset $U$ of $\mathbb{R}^2$:
    \begin{center}
		\begin{tikzpicture}
			...
			\draw[dashed] plot[smooth cycle] coordinates{(4.5,1.5) (3.75,2.6) (2.5,2.91) (1.25,2.6) (0.5,1.5) (1.25,0.6) (2.5,1) (3.75,0.4)};
		...
		\end{tikzpicture}
	\end{center}
    Consider the cotangent vector $df_p$ at point $p$ for some smooth function $f : U \to \mathbb{R}$. It is a linear functional d$f_p : \text{T}_p U \to \mathbb{R}$ defined by d$f_p(\vec{u}) = \nabla f(p) \cdot \vec{u}$ for all $\vec{u} \in \text{T}_p U$. Here T$_p U$ is the tangent space of $U$ at point $p$, which is a vector space over $\mathbb{R}$. Note that both $\vec{u}$ and $\nabla f(p)$ are depending on the choice of a coordinate system. However, d$f_p$ is independent of any choice of coordinate system. In normal calculus, d$f_p$ is called the \emph{first partial derivative} of $f$ at point $p$, and normally we write it as $\frac{\partial f}{\partial x}(p)$ and $\frac{\partial f}{\partial y}(p)$.
\end{example}
