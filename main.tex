...
\newcommand{\Map}[2]{\text{Map}(#1, #2)} % Map notation
...
\begin{center}
    ...
    \end{tikzcd}
\end{center}
where $\F[\;\cdot\;]$ sends set map $f : X \to Y$ to a linear map $\F[f] : \F[X] \to \F[Y]$. 

$\F[\;\cdot\;]$ is an example of functors.
...
\begin{example}
    ...
    \begin{center}
        \begin{tikzcd}
            \Set \arrow[r, "{\F[\;\cdot\;]}", yshift=0.5ex] & \Vect_{\F} \arrow[l, "|\;\cdot\;|", yshift=-0.5ex]
        \end{tikzcd}
    \end{center}
    where $\F[\;\cdot\;]$ sends set $X$ to the free vector space $\F[X]$ generated by $X$, and a set map $f : X \to Y$ to the linear map $\F[f] : \F[X] \to \F[Y]$ induced by $f$. The functor $|\;\cdot\;|$ sends a vector space $V$ to its underlying set $|V|$, and a linear map $\phi : V \to W$ to the set map $|\phi| : |V| \to |W|$ induced by $\phi$.

    The $\F[\;\cdot\;]$ is called the \emph{free functor}, specifically it is the \emph{free vector space functor}. The $|\;\cdot\;|$ is called the \emph{underlying functor} or \emph{forgetful functor}.
\end{example}
...
\begin{example}
    The free functor $\F[\;\cdot\;] : \Set \to \Vect_{\F}$ is a left adjoint of the underlying functor $|\;\cdot\;| : \Vect_{\F} \to \Set$. This is exactly the universal property of free vector space over a set.
\end{example}
...
\begin{center}
    ...
    \end{tikzcd}
\end{center}
where $- \times X$ is the product endofunctor from $\Set$ to $\Set$:
\begin{center}
    \begin{tikzcd}[row sep=normal]
    ...
    \end{tikzcd}
    \begin{tikzcd}[row sep=normal]
        \Set(X, Y) \arrow[dd] & Y \arrow[dd, "f"] \\
        \phantom{*} & \arrow[l, mapsto, shorten <= 2ex, shorten >= 2ex] \\
        \Set(X, Z) & Z 
    \end{tikzcd}
\end{center}

The $\Set(X, -)$ is the hom endofunctor from $\Set$ to $\Set$:
